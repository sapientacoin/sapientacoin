...
uint256 hashGenesisBlock("0x000004ad52de3b315605fc58e8e45cb2c126f1424c435961ec9398bfb7fad4ad"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SapientaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SapientaCoin Signed Message:\n";
...
{
    // SapientaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 175200 blocks, which will occur approximately every 1 year
    nSubsidy >>= (nHeight / 175200);
...
static const int64 nTargetSpacing = 180; // 3 Minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 480 blocks daily
...
{
        static const int64 			BlocksTargetSpacing  		              = 180;
...
void ThreadScriptCheck() {
    RenameThread("sapientacoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SapientaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SapientaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000004ad52de3b315605fc58e8e45cb2c126f1424c435961ec9398bfb7fad4ad");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block        
        const char* pszTimestamp = "https://bitcointalk.org/index.php?topic=663102.0";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x2caee5c250937712df509d471c4d091f6756131e8ec033148ebfa0f9922ea914"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x5A, 0x5B, 0x5C, 0x5D };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SapientaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SapientaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SapientaCoinMiner(CWallet *pwallet)
{
    printf("SapientaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sapientacoin-miner");
    ...
    try { loop {
        ...
        printf("Running SapientaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SapientaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SapientaCoinMiner, pwallet));
}
